...
(defn maze
  "Returns a random maze carved out of walls; walls is a set of 2-item sets
   #{a b} where a and b are locations. The returned maze is a set of the
   remaining walls. "
  [walls]
...
(defn hex-grid
  [w h]
  (let [verticies (set (for [y (range h) x (range (if (odd? y) 1 0) (* 2 w) 2)]
                         [x y]))
        deltas [[2 0] [1 1] [-1 1]]]
    (set (for [v verticies d deltas f [+ -]
               :let [w (verticies (map f v d))]
               :when w]
           #{v w}))))
